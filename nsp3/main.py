...
def main(args):
    ...
    for header, body in read_fasta(args.i):
        ...
        if not all(matched_residues):
            ...
            log.error(f"Sequence contains invalid residue characters characters")
            log.error(f"Residues: {invalid_seq[invalid_res]}\nHeader: {invalid_header}\nSeq: {''.join(invalid_seq)}")
            sys.exit("2Sequence contains wrong residue characters")
